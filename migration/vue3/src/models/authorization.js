...
export function normalizeAuthorizationContext(value) {
  ...
  return {
    ...
    deniedPermissions: normalizeStringCollection(
  ...
  return {
    ...
    userName:
      typeof value.user_name === 'string'
        ? value.user_name
        : '',
    collegeNumber: asNullableString(
...
export function normalizePermissionOverride(value) {
  ...
  const permission = normalizeSecurityPermission(
    value.permission
  )

  const effect = value.effec
...
export function normalizeUserAuthorization(value) {
  ...
  return {
    user,
    assignedRoles: normalizeSecurityRoles(
      value.assignedRoles
    ),
    ...
    context: normalizeAuthorizationContext(
      value.contex
    )
  }
}
...
export function normalizeOverrideInput(value) {
  ...
  for (const item of asArray(value)) {
    ...
    const permissionId = asPositiveInteger(
      item.permissionId
    )

    const effect = item.effec
