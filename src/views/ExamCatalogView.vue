  ...
  <section class="exam-catalog-page">
    ...
    <section class="exam-catalog-layout" :class="{ 'groups-collapsed': groupsCollapsed }">
      <aside v-if="!groupsCollapsed" class="exam-group-panel">
        <header class="bio-nexus-panel-heading"><div class="bio-nexus-panel-heading-copy"><p class="bio-nexus-panel-eyebrow">Catálogo</p><h3 class="bio-nexus-panel-title">Grupos de exámenes</h3></div><div class="group-header-actions"><button type="button" class="panel-collapse panel-edge-toggle" title="Ocultar grupos" aria-label="Ocultar grupos" @click="groupsCollapsed=true"><BioNexusIcon name="chevron_left" :size="20" /></button><BioNexusActionButton v-if="canCreate" icon="add" icon-only shape="rounded" size="md" variant="primary" label="Nuevo grupo" :disabled="saving" @click="groupDialog?.openCreate()" /></div></header>
        ...
        <ul v-else class="exam-group-list"><li v-for="group in filteredGroups" :key="group.id" @contextmenu.prevent.stop="openGroupContextMenu($event,group)"><button type="button" :class="{selected:selectedGroup?.id===group.id,inactive:group.annulled}" @click="selectGroup(group)"><span><strong>{{ group.description }}</strong><small>{{ group.its_exam ? 'Análisis clínico' : 'Grupo auxiliar' }}</small></span><em>{{ group.annulled ? 'Desactivado' : 'Activo' }}</em></button><BioNexusActionButton v-if="canUpdate" icon="edit" icon-only shape="circle" size="sm" variant="subtle" label="Editar grupo" @click="groupDialog?.openEdit(group)" /></li></ul>
        <div v-if="selectedGroup" class="exam-group-actions"><button v-if="canChangeStatus" class="bio-nexus-action bio-nexus-action-secondary" @click="stateDialog?.open(selectedGroup,'grupo')">{{ selectedGroup.annulled ? 'Activar grupo' : 'Desactivar grupo' }}</button></div>
      </aside>
      ...
      <main class="exam-grid-panel">
        <header class="exam-grid-heading bio-nexus-panel-heading"><div class="bio-nexus-panel-heading-copy"><p class="bio-nexus-panel-eyebrow">Exámenes del grupo</p><h3 class="bio-nexus-panel-title">{{ selectedGroupTitle }}</h3></div></header>
        ...
        <BioNexusDataGrid v-else :row-data="gridRows" :column-defs="columnDefs" :default-col-def="defaultColDef" :components="gridComponents" :get-row-id="getRowId" :row-selection="rowSelection" :search-enabled="true" v-model:search-model-value="examSearch" search-placeholder="Buscar examen" :refresh-enabled="true" :refreshing="loadingExams" :refresh-disabled="saving" :page-size="10" :page-size-selector="[10,20,50,100]" :min-grid-height="360" :max-grid-height="620" @refresh="loadExams" @row-context-menu="openExamContextMenu" @grid-ready="rememberGrid">
          ...
          <template #actions><BioNexusActionButton v-if="canUpdate" icon="edit" label="Ajustar precios" variant="secondary" :disabled="saving||selectedGroup.annulled" @click="priceAdjustmentDialog?.open(selectedRows.length ? 'selected' : 'group')" /><BioNexusActionButton v-if="canCreate" icon="add" icon-only shape="rounded" size="md" variant="primary" label="Nuevo examen" :disabled="saving||selectedGroup.annulled" @click="examDialog?.openCreate()" /></template>
        </BioNexusDataGrid>
      </main>
    </section>
    ...
    <ExamDialog ref="examDialog" :saving="saving" :can-create="canCreate" :can-update="canUpdate" :taxes="taxes" :tariffs="tariffs" :group="selectedGroup" @submit="saveExam" />
    <ExamStateDialog ref="stateDialog" :saving="saving" @confirm="saveState" /><ExamBulkStateDialog ref="bulkStateDialog" :saving="saving" @confirm="confirmBulkState" /><ExamDetailDialog ref="detailDialog" :taxes="taxes" :tariffs="tariffs" /><ExamWorksheetDialog ref="worksheetDialog" :saving="saving" :can-update="canUpdate" @save="saveWorksheet" /><ExamResultGridDesignerDialog ref="resultGridDesignerDialog" @save="saveResultGridFormat" /><ExamPriceAdjustmentDialog ref="priceAdjustmentDialog" :saving="saving" :group="selectedGroup" :groups="groups" :records="exams" :selected-records="selectedRows" :tariffs="tariffs" @confirm="applyPriceAdjustment" />
  </section>
</template>
<script setup>
import { formatUsdPrice } from "@/services/regionalFormatter";
...
import ExamStateDialog from "@/components/examcatalog/ExamStateDialog.vue";
import ExamPriceAdjustmentDialog from "@/components/examcatalog/ExamPriceAdjustmentDialog.vue";import ExamBulkStateDialog from "@/components/examcatalog/ExamBulkStateDialog.vue";import ExamDetailDialog from "@/components/examcatalog/ExamDetailDialog.vue";import ExamWorksheetDialog from "@/components/examcatalog/ExamWorksheetDialog.vue";const ExamResultGridDesignerDialog=defineAsyncComponent(()=>import("@/components/examcatalog/designer/ExamResultGridDesignerDialog.vue"));
import { useBioNexusToast } from "@/composables/useBioNexusToast";
import { adjustExamPrices, bulkUpdateExams, createExam, createExamGroup, examCatalogError, getExam, getExamGroups, getExamsByGroup, getCatalogTariffs, getTaxes, updateExam, updateExamGroup } from "@/services/examCatalogService";
...
const gridRows=computed(()=>{const q=examSearch.value.trim().toLowerCase();return exams.value.filter(x=>!q||[x.description,x.abbreviation,x.special_test?"especial":"",x.annulled?"desactivado":"activo"].some(v=>String(v).toLowerCase().includes(q))).map(x=>({...x,isActive:!x.annulled,isSpecial:Boolean(x.special_test)}))});
...
const tariffColumns=computed(()=>tariffs.value.filter(t=>Number(t.position)>=1&&Number(t.position)<=6).sort((a,b)=>Number(a.position)-Number(b.position)).map(t=>({field:`cost${Number(t.position)}`,headerName:`${t.name} (${regionalSettings.settings.base_currency_symbol})`,width:125,type:"numericColumn",cellClass:"exam-price-cell",headerClass:"exam-price-header",valueFormatter:x=>formatUsdPrice(x.value,regionalSettings.settings)})));
const columnDefs=computed(()=>[
  {field:"description",headerName:"Descripción",minWidth:280,width:330,pinned:"left",lockPinned:true,suppressMovable:true,cellClass:"exam-description-pinned"},
  ...
  ...tariffColumns.value,
  {field:"isActive",headerName:"Estado",width:155,cellClass:"exam-toggle-cell",headerClass:"exam-toggle-header",filter:BioNexusOptionFilter,filterParams:{options:[{value:true,label:"Activo"},{value:false,label:"Desactivado"}]},cellRenderer:"BioNexusGridToggleCell",cellRendererParams:{onLabel:"Activo",offLabel:"Desactivado",disabled:()=>!canChangeStatus.value||saving.value,onToggle:row=>stateDialog.value?.open(row,"examen")}},
  {colId:"actions",headerName:"Acciones",width:196,minWidth:196,maxWidth:196,pinned:"right",lockPinned:true,suppressMovable:true,sortable:false,filter:false,resizable:false,cellRenderer:"BioNexusGridActionsCell",cellRendererParams:{actions:[{key:"view",label:"Ver",icon:"visibility",onClick:row=>detailDialog.value?.show(row,selectedGroup.value)},{key:"worksheet",label:"Hoja de trabajo",icon:"description",visible:()=>canUpdate.value,onClick:row=>openWorksheet(row)},{key:"grid-designer",label:"Diseñador matricial",icon:"grid_on",visible:()=>canUpdate.value,onClick:row=>openResultGridDesigner(row)},{key:"edit",label:"Editar",visible:()=>canUpdate.value,disabled:()=>saving.value,onClick:row=>examDialog.value?.openEdit(row)}]}}
]);
const contextMenuItems=computed(()=>{const state=contextMenuState.value,row=state.row;if(!row)return[];if(state.type==="grupo")return[{key:"edit",icon:"edit",label:"Editar grupo",visible:canUpdate.value,action:()=>groupDialog.value?.openEdit(row)},{key:row.annulled?"activate":"deactivate",icon:row.annulled?"activate":"deactivate",label:row.annulled?"Activar grupo":"Desactivar grupo",variant:row.annulled?undefined:"danger",visible:canChangeStatus.value,action:()=>stateDialog.value?.open(row,"grupo")}];const rows=selectedRows.value.length?selectedRows.value:[row],ids=rows.map(item=>item.id),hasActive=rows.some(item=>item.annulled!==true),hasInactive=rows.some(item=>item.annulled===true),hasSpecial=rows.some(item=>Boolean(item.special_test)),hasRegular=rows.some(item=>!Boolean(item.special_test));return[{key:"view",icon:"visibility",label:"Ver examen",visible:ids.length===1,action:()=>detailDialog.value?.show(rows[0],selectedGroup.value)},{key:"worksheet",icon:"description",label:"Hoja de trabajo",visible:canUpdate.value&&ids.length===1,action:()=>openWorksheet(rows[0])},{key:"grid-designer",icon:"grid_on",label:"Diseñador matricial",visible:canUpdate.value&&ids.length===1,action:()=>openResultGridDesigner(rows[0])},{key:"price-adjustment",icon:"edit",label:ids.length>1?`Ajustar precios de ${ids.length} seleccionados`:"Ajustar precios",visible:canUpdate.value,action:()=>priceAdjustmentDialog.value?.open("selected")},{key:"edit",icon:"edit",label:"Editar examen",visible:canUpdate.value&&ids.length===1,action:()=>examDialog.value?.openEdit(rows[0])},{key:"activate",icon:"activate",label:ids.length===1?"Activar examen":`Activar ${ids.length} seleccionado(s)`,visible:canChangeStatus.value&&hasInactive,action:()=>openBulkState(rows,false)},{key:"deactivate",icon:"deactivate",label:ids.length===1?"Desactivar examen":`Desactivar ${ids.length} seleccionado(s)`,variant:"danger",visible:canChangeStatus.value&&hasActive,action:()=>openBulkState(rows,true)},{key:"special-on",icon:"activate",label:ids.length===1?"Marcar como prueba especial":"Marcar seleccionados como prueba especial",visible:canUpdate.value&&hasRegular,action:()=>runBulk(ids,{special_test:true})},{key:"special-off",icon:"deactivate",label:ids.length===1?"Quitar prueba especial":"Quitar prueba especial a seleccionados",visible:canUpdate.value&&hasSpecial,action:()=>runBulk(ids,{special_test:false})}]});
...
function replaceExam(row){const index=exams.value.findIndex(x=>x.id===row.id);if(index>=0){const next=[...exams.value];next[index]=row;exams.value=next;return}exams.value=[...exams.value,row]}
...
function openBulkState(records,annulled){bulkStateDialog.value?.open(records,annulled)}async function confirmBulkState(records,annulled){bulkStateDialog.value?.clearError();try{await runBulk(records.map(item=>item.id),{annulled});bulkStateDialog.value?.close()}catch(error){bulkStateDialog.value?.setError(examCatalogError(error,"No fue posible cambiar el estado."))}}async function runBulk(ids,changes){if(saving.value||ids.length===0)return;saving.value=true;try{const result=await bulkUpdateExams(ids,changes);await loadExams();toast.success(`${result.updated} examen(es) actualizado(s).`)}catch(e){toast.error(examCatalogError(e,"No fue posible aplicar el cambio masivo."))}finally{saving.value=false}}
