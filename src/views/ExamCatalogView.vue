<template>
  <section class="exam-catalog-page">
    ...
    <section class="exam-catalog-layout" :class="{ 'groups-collapsed': groupsCollapsed }">
      ...
      <main class="exam-grid-panel">
        ...
        <BioNexusDataGrid v-else :row-data="gridRows" :column-defs="columnDefs" :default-col-def="defaultColDef" :components="gridComponents" :get-row-id="getRowId" :row-selection="rowSelection" :search-enabled="true" v-model:search-model-value="examSearch" search-placeholder="Buscar examen" :refresh-enabled="true" :refreshing="loadingExams" :refresh-disabled="saving" :page-size="10" :page-size-selector="[10,20,50,100]" :min-grid-height="360" :max-grid-height="620" @refresh="loadExams" @row-context-menu="openExamContextMenu" @grid-ready="rememberGrid">
          ...
          <template #actions><button v-if="canUpdate" class="bio-nexus-action bio-nexus-action-secondary" :disabled="saving||selectedGroup.annulled" @click="priceAdjustmentDialog?.open(selectedRows.length ? 'selected' : 'group')"><BioNexusActionIcon action="edit" />Ajustar precios</button><button v-if="canCreate" class="bio-nexus-action bio-nexus-action-primary" :disabled="saving||selectedGroup.annulled" @click="examDialog?.openCreate()"><BioNexusActionIcon action="create" />Nuevo examen</button></template>
        </BioNexusDataGrid>
      </main>
    </section>
    ...
    <ExamStateDialog ref="stateDialog" :saving="saving" @confirm="saveState" /><ExamBulkStateDialog ref="bulkStateDialog" :saving="saving" @confirm="confirmBulkState" /><ExamDetailDialog ref="detailDialog" :taxes="taxes" :tariffs="tariffs" /><ExamWorksheetDialog ref="worksheetDialog" :saving="saving" :can-update="canUpdate" @save="saveWorksheet" /><ExamResultGridDesignerDialog ref="resultGridDesignerDialog" :saving="saving" :can-update="canUpdate" @save="saveResultGridFormat" /><ExamPriceAdjustmentDialog ref="priceAdjustmentDialog" :saving="saving" :group="selectedGroup" :groups="groups" :records="exams" :selected-records="selectedRows" :tariffs="tariffs" @confirm="applyPriceAdjustment" />
  </section>
</template>
<script setup>
...
const contextMenuItems=computed(()=>{const state=contextMenuState.value,row=state.row;if(!row)return[];if(state.type==="grupo")return[{key:"edit",icon:"edit",label:"Editar grupo",visible:canUpdate.value,action:()=>groupDialog.value?.openEdit(row)},{key:"state",icon:row.annulled?"activate":"deactivate",label:row.annulled?"Activar grupo":"Inactivar grupo",visible:canChangeStatus.value,action:()=>stateDialog.value?.open(row,"grupo")}];const rows=selectedRows.value.length?selectedRows.value:[row],ids=rows.map(item=>item.id),hasActive=rows.some(item=>item.annulled!==true),hasInactive=rows.some(item=>item.annulled===true),hasSpecial=rows.some(item=>Boolean(item.special_test)),hasRegular=rows.some(item=>!Boolean(item.special_test));return[{key:"view",icon:"visibility",label:"Ver examen",visible:ids.length===1,action:()=>detailDialog.value?.show(rows[0],selectedGroup.value)},{key:"worksheet",icon:"description",label:"Hoja de trabajo",visible:canUpdate.value&&ids.length===1,action:()=>openWorksheet(rows[0])},{key:"grid-designer",icon:"grid_on",label:"Diseñador matricial",visible:canUpdate.value&&ids.length===1,action:()=>openResultGridDesigner(rows[0])},{key:"price-adjustment",icon:"edit",label:ids.length>1?`Ajustar precios de ${ids.length} seleccionados`:"Ajustar precios",visible:canUpdate.value,action:()=>priceAdjustmentDialog.value?.open("selected")},{key:"edit",icon:"edit",label:"Editar examen",visible:canUpdate.value&&ids.length===1,action:()=>examDialog.value?.openEdit(rows[0])},{key:"activate",icon:"activate",label:ids.length===1?"Activar examen":`Activar ${ids.length} seleccionado(s)`,visible:canChangeStatus.value&&hasInactive,action:()=>openBulkState(rows,false)},{key:"deactivate",icon:"deactivate",label:ids.length===1?"Inactivar examen":`Inactivar ${ids.length} seleccionado(s)`,visible:canChangeStatus.value&&hasActive,action:()=>openBulkState(rows,true)},{key:"special-on",icon:"activate",label:ids.length===1?"Marcar como prueba especial":"Marcar seleccionados como prueba especial",visible:canUpdate.value&&hasRegular,action:()=>runBulk(ids,{special_test:true})},{key:"special-off",icon:"deactivate",label:ids.length===1?"Quitar prueba especial":"Quitar prueba especial a seleccionados",visible:canUpdate.value&&hasSpecial,action:()=>runBulk(ids,{special_test:false})}]});
