...
import ExamStateDialog from "@/components/examcatalog/ExamStateDialog.vue";import ExamBulkStateDialog from "@/components/examcatalog/ExamBulkStateDialog.vue";import ExamDetailDialog from "@/components/examcatalog/ExamDetailDialog.vue";import ExamWorksheetDialog from "@/components/examcatalog/ExamWorksheetDialog.vue";const ExamResultGridDesignerDialog=defineAsyncComponent(()=>import("@/components/examcatalog/experimental/ExamResultGridDesignerDialog.vue"));
import { useBioNexusToast } from "@/composables/useBioNexusToast";
import { bulkUpdateExams, createExam, createExamGroup, examCatalogError, getExam, getExamGroups, getExamsByGroup, getCatalogTariffs, getTaxes, updateExam, updateExamGroup } from "@/services/examCatalogService";
import { useAuthorizationStore } from "@/stores/authorization";
const auth=useAuthorizationStore(),toast=useBioNexusToast();
const groups=ref([]),exams=ref([]),taxes=ref([]),tariffs=ref([]),selectedGroup=ref(null),selectedRows=ref([]),loadingGroups=ref(false),loadingExams=ref(false),saving=ref(false),loadError=ref(""),groupSearch=ref(""),examSearch=ref(""),globalSearchText=ref(""),groupsCollapsed=ref(false),groupDialog=ref(null),examDialog=ref(null),stateDialog=ref(null),bulkStateDialog=ref(null),detailDialog=ref(null),worksheetDialog=ref(null),resultGridDesignerDialog=ref(null),globalSearch=ref(null),contextMenu=ref(null),gridApi=ref(null),contextMenuState=ref({open:false,x:0,y:0,type:"",row:null});
...
const contextMenuItems=computed(()=>{const state=contextMenuState.value,row=state.row;if(!row)return[];if(state.type==="grupo")return[{key:"edit",icon:"edit",label:"Editar grupo",visible:canUpdate.value,action:()=>groupDialog.value?.openEdit(row)},{key:"state",icon:row.annulled?"activate":"deactivate",label:row.annulled?"Activar grupo":"Inactivar grupo",visible:canChangeStatus.value,action:()=>stateDialog.value?.open(row,"grupo")}];const rows=selectedRows.value.length?selectedRows.value:[row],ids=rows.map(item=>item.id),hasActive=rows.some(item=>item.annulled!==true),hasInactive=rows.some(item=>item.annulled===true),hasSpecial=rows.some(item=>Boolean(item.special_test)),hasRegular=rows.some(item=>!Boolean(item.special_test));return[{key:"view",icon:"visibility",label:"Ver examen",visible:ids.length===1,action:()=>detailDialog.value?.show(rows[0],selectedGroup.value)},{key:"worksheet",icon:"description",label:"Hoja de trabajo",visible:canUpdate.value&&ids.length===1,action:()=>openWorksheet(rows[0])},{key:"grid-designer",icon:"grid_on",label:"Diseñador matricial",visible:canUpdate.value&&ids.length===1,action:()=>openResultGridDesigner(rows[0])},{key:"edit",icon:"edit",label:"Editar examen",visible:canUpdate.value&&ids.length===1,action:()=>examDialog.value?.openEdit(rows[0])},{key:"activate",icon:"activate",label:ids.length===1?"Activar examen":`Activar ${ids.length} seleccionado(s)`,visible:canChangeStatus.value&&hasInactive,action:()=>openBulkState(rows,false)},{key:"deactivate",icon:"deactivate",label:ids.length===1?"Inactivar examen":`Inactivar ${ids.length} seleccionado(s)`,visible:canChangeStatus.value&&hasActive,action:()=>openBulkState(rows,true)},{key:"special-on",icon:"activate",label:ids.length===1?"Marcar como prueba especial":"Marcar seleccionados como prueba especial",visible:canUpdate.value&&hasRegular,action:()=>runBulk(ids,{special_test:true})},{key:"special-off",icon:"deactivate",label:ids.length===1?"Quitar prueba especial":"Quitar prueba especial a seleccionados",visible:canUpdate.value&&hasSpecial,action:()=>runBulk(ids,{special_test:false})}]});
