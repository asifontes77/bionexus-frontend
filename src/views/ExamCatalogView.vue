<template>
  <section class="exam-catalog-page">
    ...
    <ExamStateDialog ref="stateDialog" :saving="saving" @confirm="saveState" /><ExamDetailDialog ref="detailDialog" :taxes="taxes" /><ExamWorksheetDialog ref="worksheetDialog" :saving="saving" :can-update="canUpdate" @save="saveWorksheet" /><ExamResultDesignerDialog ref="resultDesignerDialog" :saving="saving" :can-update="canUpdate" @save="saveResultFormat" /><ExamResultGridDesignerDialog ref="resultGridDesignerDialog" :saving="saving" :can-update="canUpdate" @save="saveResultGridFormat" />
  </section>
</template>
<script setup>
...
import ExamStateDialog from "@/components/examcatalog/ExamStateDialog.vue";import ExamDetailDialog from "@/components/examcatalog/ExamDetailDialog.vue";import ExamWorksheetDialog from "@/components/examcatalog/ExamWorksheetDialog.vue";import ExamResultDesignerDialog from "@/components/examcatalog/ExamResultDesignerDialog.vue";import ExamResultGridDesignerDialog from "@/components/examcatalog/experimental/ExamResultGridDesignerDialog.vue";
...
const columnDefs=computed(()=>[
  ...
  {colId:"actions",headerName:"Acciones",width:196,minWidth:196,maxWidth:196,pinned:"right",lockPinned:true,suppressMovable:true,sortable:false,filter:false,resizable:false,cellRenderer:"BioNexusGridActionsCell",cellRendererParams:{actions:[{key:"view",label:"Ver",icon:"visibility",onClick:row=>detailDialog.value?.show(row,selectedGroup.value)},{key:"worksheet",label:"Hoja de trabajo",icon:"description",visible:()=>canUpdate.value,onClick:row=>openWorksheet(row)},{key:"result-designer",label:"Diseñador de resultado",icon:"dashboard_customize",visible:()=>canUpdate.value,onClick:row=>openResultDesigner(row)},{key:"grid-designer",label:"Diseñador matricial (piloto)",icon:"grid_on",visible:()=>canUpdate.value,onClick:row=>openResultGridDesigner(row)},{key:"edit",label:"Editar",visible:()=>canUpdate.value,disabled:()=>saving.value,onClick:row=>examDialog.value?.openEdit(row)}]}}
]);
const contextMenuItems=computed(()=>{const state=contextMenuState.value,row=state.row;if(!row)return[];if(state.type==="grupo")return[{key:"edit",icon:"edit",label:"Editar grupo",visible:canUpdate.value,action:()=>groupDialog.value?.openEdit(row)},{key:"state",icon:row.annulled?"activate":"deactivate",label:row.annulled?"Activar grupo":"Inactivar grupo",visible:canChangeStatus.value,action:()=>stateDialog.value?.open(row,"grupo")}];const rows=selectedRows.value.length?selectedRows.value:[row],ids=rows.map(item=>item.id),hasActive=rows.some(item=>item.annulled!==true),hasInactive=rows.some(item=>item.annulled===true),hasSpecial=rows.some(item=>Boolean(item.special_test)),hasRegular=rows.some(item=>!Boolean(item.special_test));return[{key:"view",icon:"visibility",label:"Ver examen",visible:ids.length===1,action:()=>detailDialog.value?.show(rows[0],selectedGroup.value)},{key:"worksheet",icon:"description",label:"Hoja de trabajo",visible:canUpdate.value&&ids.length===1,action:()=>openWorksheet(rows[0])},{key:"result-designer",icon:"dashboard_customize",label:"Diseñador de resultado",visible:canUpdate.value&&ids.length===1,action:()=>openResultDesigner(rows[0])},{key:"grid-designer",icon:"grid_on",label:"Diseñador matricial (piloto)",visible:canUpdate.value&&ids.length===1,action:()=>openResultGridDesigner(rows[0])},{key:"edit",icon:"edit",label:"Editar examen",visible:canUpdate.value&&ids.length===1,action:()=>examDialog.value?.openEdit(rows[0])},{key:"activate",icon:"activate",label:ids.length===1?"Activar examen":`Activar ${ids.length} seleccionado(s)`,visible:canChangeStatus.value&&hasInactive,action:()=>runBulk(ids,{annulled:false})},{key:"deactivate",icon:"deactivate",label:ids.length===1?"Inactivar examen":`Inactivar ${ids.length} seleccionado(s)`,visible:canChangeStatus.value&&hasActive,action:()=>runBulk(ids,{annulled:true})},{key:"special-on",icon:"activate",label:ids.length===1?"Marcar como prueba especial":"Marcar seleccionados como prueba especial",visible:canUpdate.value&&hasRegular,action:()=>runBulk(ids,{special_test:true})},{key:"special-off",icon:"deactivate",label:ids.length===1?"Quitar prueba especial":"Quitar prueba especial a seleccionados",visible:canUpdate.value&&hasSpecial,action:()=>runBulk(ids,{special_test:false})}]});
...
async function openResultGridDesigner(row){if(!canUpdate.value||saving.value)return;try{const full=await getExam(row.id);resultGridDesignerDialog.value?.open(full)}catch(e){toast.error(examCatalogError(e,"No fue posible abrir el Diseñador matricial."))}}
async function saveResultGridFormat(payload){if(!payload?.record||!payload?.format_grid||saving.value)return;saving.value=true;try{const saved=await updateExam(payload.record.id,{format_grid:payload.format_grid});replaceExam(saved);resultGridDesignerDialog.value?.markSaved(saved);toast.success("Formato matricial guardado correctamente.")}catch(e){toast.error(examCatalogError(e,"No fue posible guardar el Formato matricial."))}finally{saving.value=false}}async function saveResultFormat(payload){if(!payload?.record||saving.value)return;saving.value=true;try{const saved=await updateExam(payload.record.id,{format_vue:payload.format_vue});replaceExam(saved);resultDesignerDialog.value?.markSaved(saved);toast.success("Formato de resultado guardado correctamente.")}catch(e){toast.error(examCatalogError(e,"No fue posible guardar el Formato de resultado."))}finally{saving.value=false}}async function saveWorksheet(payload){if(!payload?.record||saving.value)return;saving.value=true;worksheetDialog.value?.setError("");try{const saved=await updateExam(payload.record.id,{work_sheet:payload.work_sheet});replaceExam(saved);worksheetDialog.value?.markSaved(saved);toast.success("Hoja de trabajo guardada correctamente.")}catch(e){worksheetDialog.value?.setError(examCatalogError(e,"No fue posible guardar la Hoja de trabajo."))}finally{saving.value=false}}async function saveState(row,label){saving.value=true;stateDialog.value?.clearError();try{const saved=label==="grupo"?await updateExamGroup(row.id,{annulled:!row.annulled}):await updateExam(row.id,{annulled:!row.annulled});if(label==="grupo")replaceGroup(saved);else replaceExam(saved);stateDialog.value?.close();toast.success(saved.annulled?"Registro inactivado correctamente.":"Registro activado correctamente.")}catch(e){stateDialog.value?.setError(examCatalogError(e,"No fue posible cambiar el estado."))}finally{saving.value=false}}
