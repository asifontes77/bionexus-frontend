<template>
    <v-container pa-0 fluid fill-height class="align-start pa-10">
        ...
        <v-tabs v-model="tab" background-color="transparent" color="primary">
            ...
            <v-tab>
                <v-icon color="secondary" left>mdi-checkbox-marked-circle-plus-outline</v-icon>
                Registrar
            </v-tab>
            <v-tab-item>
        ...
        <div class="ml-6">
            <v-btn v-if="tab === 3" color="secondary" class="px-6" @click="toRegister()">
                <v-icon left>mdi-arrow-collapse-down</v-icon>
                Registrar
            </v-btn>
            <v-alert v-show="valid" outlined text transition="scale-transition" color="secondary" class="mt-10">
                <v-icon large>mdi-alert-circle-outline</v-icon>
                <div v-html="textAlert" class="ml-16 mt-n10"></div>
            </v-alert>
        </div>
        <end-admission-dialog v-if="newPatient" v-model="activeCerrarIngreso" :new-patient="newPatientEnd"
            @cerrar_ingreso="cerrarIngreso()" />
        <div id="printV" :style="voucherOculto ? 'display: none' : 'display: block'"></div>  
        <print-receipt v-if="patientIdR" :id="patientIdR" :can-dolares="canDolares" @close-print="patientIdR=null" />  
    </v-container>
</template>
...
export default {
    ...
    mixins: [examsListMixin, pacientsMixin, laboratoryMixin, functionMixin, accountsreceivableMixin],
    components: { InformationPatiente, SelectExams, TypePayment, RegisterPatient, EndAdmissionDialog, PrintReceipt },
    data() {
        return {
            typePaymentKey: 0,
    ...
    methods: {  
        ...
        async setInvoiceApply(patientId) {
            ...
            var index = 0
            var listExaInvoice = []
            var wayPayBs = this.newPatient.way_pay_dollars * 100 / this.newPatient.total_dollars
            await this.asyncForEach(this.newExams, async (item) => {
                ...
                if (noRows === this.rowsVoucher) {
                    ...
                    footPayments = await this.calculateTotals(footPayments, this.canDolares, listExaInvoice, wayPayBs)
                    listExaInvoice = []
        ...
        validateRecords() {
            ...
            if (this.newPatient.name === '') {
                validate = false
                texto += '· Nombre de paciente no puede quedar en blanco<br>'
            }
            if (this.newPatient.birth_date === '') {
                validate = false
                texto += '· La edad del paciente no puede quedar en blanco<br>'
            }
            if (this.newPatient.email_sent === 0) {
                if (this.newPatient.email === '') {
                    validate = false
                    texto += '· El Email no puede quedar en blanco<br>'
                }
            }
            if (this.newExams.length === 0) {
                validate = false
                texto += '· Debe seleccionar exámenes<br>'
            }
            if (this.newPatient.client_id === 1) {
                if (this.wayPay.length === 0) {
                    validate = false
                    texto += '· Seleccione forma de pago<br>'
                }
                console.log('this.newPatient: ', this.newPatient)
                if (parseFloat(this.newPatient.total_dollars) !== 0) {
                    console.log('this.newPatient.total_dollars: ', this.newPatient.total_dollars)
                    console.log('this.newPatient.total_canceled: ', this.newPatient.total_canceled)
                    if (parseFloat(this.newPatient.total_dollars) !== parseFloat(this.newPatient.total_canceled)) {
                        validate = false
                        texto += '· Saldo pendiente por cancelar<br>'
                    }
                } else {
                    if (parseFloat(this.newPatient.total) !== Math.round(parseFloat(this.newPatient.total_canceled))) {
                        validate = false
                        texto += '· Saldo pendiente por cancelar<br>'
                    }
                }
            ...
            if (this.register.printInvoice) {
                if (this.newPatient.business_name === '') {
                    validate = false
                    texto += '· Razón social no puede quedar en blanco<br>'
                }
                if (this.newPatient.rif === '') {
                    validate = false
                    texto += '· Registro de información fiscal no puede quedar en blanco<br>'
                }
                if (this.newPatient.address === '') {
                    validate = false
                    texto += '· Domicilio no puede quedar en blanco<br>'
                }
            }
            if (this.register.printVoucher) {
                if (this.newPatient.business_name === '') {
        ...
        async colocaFact(itemC, fechaActual, miDiv, patientTmp) {
            ...
            tipoFactura = tipoFactura.replace('[razon-social]', printVoices.business_name)
            tipoFactura = tipoFactura.replace('[rif]', printVoices.rif)
            tipoFactura = tipoFactura.replace('[domicilio]', printVoices.address)
            tipoFactura = tipoFactura.replace('[position]', patientTmp.patient_position)
            ...
            await this.asyncForEach(printVoices.foot_payments, async (item) => {
                if (item.active) {
                    textFoot += `<tr>
                        ...
                        if (item.sobre !== null) {
                            textFoot += ` SOBRE ${this.formatNumber(parseFloat(item.sobre * this.dollarValue.value))}`
                        }
                        textFoot += `</td>
                        <td style="padding: 5px 10px; border-bottom: 2px solid black; border-right: 2px solid black; border-left: 2px solid black; text-align: right;">
                            ${this.formatNumber(parseFloat(item.total * this.dollarValue.value))}
                        </td>
                    </tr>`
                }
                ini += 1
