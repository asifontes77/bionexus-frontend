  ...
  <section class="worksheet-groups-page">
    ...
    <section class="bio-nexus-administrative-directory">
      <BioNexusDataGrid class="worksheet-groups-grid" :row-data="rows" :column-defs="columnDefs" :default-col-def="defaultColDef" :components="gridComponents" :get-row-id="getRowId" :quick-filter-text="searchText" :search-enabled="true" v-model:search-model-value="searchText" search-placeholder="Buscar grupo" :refresh-enabled="true" :refreshing="loading" :refresh-disabled="saving" :page-size="10" :page-size-selector="[10,20,50,100]" :min-grid-height="300" :max-grid-height="560" empty-text="No existen grupos de hojas de trabajo." @refresh="loadRows" @row-context-menu="openContextMenu">
        <template #actions><button v-if="canCreate" class="bio-nexus-action bio-nexus-action-primary bio-nexus-grid-icon-action" :disabled="loading || saving" @click="formDialog?.openCreate()" title="Nuevo grupo" aria-label="Nuevo grupo"><BioNexusActionIcon action="create" /></button></template>
      </BioNexusDataGrid>
...
const contextItems=computed(()=>{const row=contextState.value.row;if(!row)return[];return[{key:'edit',label:'Editar',icon:'edit',visible:canUpdate.value||canManageItems.value,disabled:saving.value,action:()=>openEdit(row)},{key:'state',label:row.annulled?'Activar grupo':'Desactivar grupo',icon:row.annulled?'activate':'deactivate',visible:canState.value,disabled:saving.value,action:()=>stateDialog.value?.open(row)}]})
...
async function saveState(row){saving.value=true;stateDialog.value?.clearError();try{await updateWorksheetGroup(row.id,{annulled:!row.annulled});stateDialog.value?.close();toast.success(row.annulled?'Grupo activado.':'Grupo desactivado.');await loadRows()}catch(error){stateDialog.value?.setError(worksheetGroupsError(error,'No fue posible cambiar el estado.'))}finally{saving.value=false}}
