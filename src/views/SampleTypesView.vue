...
async function saveState(row){if(saving.value)return;saving.value=true;stateDialog.value?.clearError();try{const saved=await changeSampleTypeStatus(row.id,!row.annulled);if(!saved)throw new Error('El Backend no devolvio un registro valido.');replaceRow(saved);stateDialog.value?.close();toast.success(row.annulled?'Tipo de muestra activado correctamente.':'Tipo de muestra inactivado correctamente.')}catch(error){stateDialog.value?.setError(sampleTypeError(error,'No fue posible cambiar el estado.'))}finally{saving.value=false}}
onMounted(loadRows)
</script>
<style scoped>
