...
describe("Price adjustment compact layout",()=>{
  it("usa acento homogeneo en las tres secciones",()=>{
    assert.match(dialog,/title="Exámenes incluidos"[^>]*variant="accent"/);
    assert.match(dialog,/title="Operación y tarifas"[^>]*variant="accent"/);
    assert.match(dialog,/title="Vista previa"[^>]*variant="accent"/);
    assert.ok(!dialog.match(/title="Vista previa"[^>]*variant="soft"/));
  });
