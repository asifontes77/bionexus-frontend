...
export default {
  ...
  methods: {
    ...
    async createPrint() {
      this.printFormat = this.printFormat.replace(
        "[razon-social]",
      ...
      await this.asyncForEach(pages, async (itemPage) => {
        ...
        const filteredArray = listExaInvoice.filter(
          (item) => item.pageCount === itemPage.pageCount
        );
        var index = 0;
        var listDetail = "";
        subtotalT = 0;
        ...
        if (subtotalT === parseFloat(this.newPatient.subtotal)) {
          wayPayDs =
            parseFloat(this.newPatient.way_pay_dollars) *
            parseFloat(this.dollarValue.value);
        } else {
          wayPayDs =
            (subtotalT * parseFloat(this.newPatient.way_pay_dollars)) /
            parseFloat(this.newPatient.subtotal);
          wayPayDs = parseFloat(
            (wayPayDs * parseFloat(this.dollarValue.value)).toFixed(2)
          );
        }
    ...
    async saveInvoice(arrayPrinting) {
      await this.asyncForEach(arrayPrinting, async (item) => {
        const invoiceId = await this.registerInvoice(
          item.footPayments,
          item.noInvoice
        );
