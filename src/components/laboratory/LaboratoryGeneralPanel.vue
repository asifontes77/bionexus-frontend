<template>
  <section class="bio-nexus-panel laboratory-panel">
    ...
    <div class="bio-nexus-section qr"><div class="bio-nexus-section-heading"><div><h4 class="bio-nexus-section-title">Código QR de contacto</h4><p class="qr-description">Configura los datos de contacto que se usarán para generar el QR institucional. La inserción del QR en resultados o documentos se habilitará posteriormente desde Documentos y plantillas.</p></div><label class="toggle"><input v-model="model.settingQR.activeQR" type="checkbox" :disabled="disabled"><span>Configurar QR</span></label></div>
...
<style scoped>.identity-section+.identity-section,.qr{margin-top:var(--bio-nexus-space-4)}.identity-section>.bio-nexus-section-title{margin-bottom:var(--bio-nexus-space-3)}.form-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:var(--bio-nexus-space-3)}.qr-fields{padding:var(--bio-nexus-space-4)}.toggle{display:inline-flex;align-items:center;gap:var(--bio-nexus-space-2);color:var(--bio-nexus-color-text-secondary);font-weight:var(--bio-nexus-font-weight-bold)}.qr-description{max-width:780px;margin:5px 0 0;color:var(--bio-nexus-color-text-secondary);font-size:var(--bio-nexus-font-size-sm);line-height:1.4}.section-error{margin:var(--bio-nexus-space-2) var(--bio-nexus-space-4) 0;color:var(--bio-nexus-color-danger,#b42318);font-size:var(--bio-nexus-font-size-xs);font-weight:var(--bio-nexus-font-weight-bold)}@media(max-width:720px){.form-grid{grid-template-columns:1fr}}</style>
