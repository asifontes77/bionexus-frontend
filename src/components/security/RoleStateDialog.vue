...
const configuration = { kicker: "Estado del rol", isInactive: role => !Boolean(role?.isActive), activateTitle: "Activar rol", deactivateTitle: "Desactivar rol", activateMessage: "El rol volverá a estar disponible para las asignaciones permitidas.", deactivateMessage: "El rol quedará desactivado y no podrá asignarse a nuevos usuarios.", label: role => role?.name || role?.code || "Rol seleccionado", dangerOnDeactivate: true };
