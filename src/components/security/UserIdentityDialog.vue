  ...
  <dialog ref="identityDialog" class="bio-nexus-dialog user-identity-dialog" tabindex="-1">
    <form class="dialog-shell" @submit.prevent="submitIdentity">
      <header class="dialog-header">
        <div><p>{{ isCreateMode ? "Nueva identidad" : "Datos administrativos" }}</p><h3>{{ isCreateMode ? "Nuevo usuario" : `Editar ${currentUser?.name || "usuario"}` }}</h3></div>
      ...
      <div class="dialog-body identity-dialog-body">
        ...
        <section class="identity-password-section">
          <header><div><span>Acceso</span><h4>{{ isCreateMode ? "Contrasena inicial" : "Cambio opcional de contrasena" }}</h4></div><p v-if="!isCreateMode">Deja ambos campos vacios para conservar la contrasena actual.</p></header>
          <div class="identity-form-grid">
        ...
        <section class="identity-password-section">
          <header><div><span>Firma</span><h4>Cambio opcional de clave de firma</h4></div><p>Deja ambos campos vacios para conservar la clave de firma actual.</p></header>
          <div class="identity-form-grid">
...
<button type="button" class="bio-nexus-action bio-nexus-action-secondary" :disabled="saving" @click="closeIdentityDialog"><BioNexusActionIcon action="cancel" />Cancelar</button>
        <button type="submit" class="bio-nexus-action bio-nexus-action-primary" :disabled="saving || validatingEmail"><BioNexusIcon :name="isCreateMode ? 'person_add' : 'save'" :size="19" />{{ saving ? "Guardando..." : isCreateMode ? "Crear usuario" : "Guardar datos" }}</button>
      </footer>
    </form>
  </dialog>
...
function show(dialogReference) { const dialog = dialogReference.value; if (!dialog || dialog.open) return; dialog.showModal(); requestAnimationFrame(() => dialog.focus({ preventScroll: true })); }
function close(dialog) { if (dialog.value?.open) dialog.value.close(); }
...
<style scoped>
.bio-nexus-dialog { box-sizing: border-box; position: fixed; inset: 0; width: min(820px, calc(100vw - 32px)); max-width: 820px; max-height: calc(100vh - 32px); margin: auto; padding: 0; border: 1px solid var(--bio-nexus-color-border-strong); border-radius: var(--bio-nexus-radius-md); background: var(--bio-nexus-color-surface); color: var(--bio-nexus-color-text); box-shadow: var(--bio-nexus-shadow-md); overflow: hidden; }
.dialog-shell { display: flex; flex-direction: column; max-height: calc(100vh - 32px); overflow: hidden; }
...
@media (max-width: 720px) { .bio-nexus-dialog { width: calc(100vw - 16px); } .identity-form-grid, .identity-media-section { grid-template-columns: 1fr; } .identity-address-field { grid-column: auto; } }
</style>
