...
const form = reactive(createEmptyForm()); const isCreateMode = computed(() => mode.value === "create"); const identityFormValid = computed(() => { const emailValid = isValidEmail(form.email); const requiredValid = Boolean(form.name.trim() && form.userName.trim() && emailValid); const passwordValid = !isCreateMode.value || Boolean(form.password) && form.password.length >= 6 && form.password === form.passwordConfirm; const signatureValid = (!form.signaturePassword || form.signaturePassword.length >= 6) && form.signaturePassword === form.signaturePasswordConfirm; return requiredValid && passwordValid && signatureValid; }); const identitySubmitDisabled = computed(() => saving.value || validatingEmail.value || !identityFormValid.value);
...
function isValidEmail(value) { const email = normalizeEmail(value); return email.length <= 100 && /^[a-z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)+$/i.test(email); }
function validateEmailFormat() { form.email = normalizeEmail(form.email); if (!form.email) { errors.email = "El correo es obligatorio."; return false; } if (!isValidEmail(form.email)) { errors.email = "Ingresa un correo valido, por ejemplo usuario@dominio.com."; return false; } delete errors.email; return true; }
async function validateEmailField() { if (!validateEmailFormat()) return false; return validateRemoteEmail(); }
async function validateRemoteEmail() { if (!validateEmailFormat()) return false; validatingEmail.value = true; try { const duplicate = await verifyUserEmail(form.email, currentUser.value?.id ?? null); if (duplicate) { errors.email = "Ya existe un usuario con ese correo electronico."; return false; } delete errors.email; return true; } catch (error) { const code = error?.message || ""; if (code === "USER_EMAIL_INVALID") errors.email = "Ingresa un correo valido, por ejemplo usuario@dominio.com."; else if (code === "USER_EMAIL_ALREADY_EXISTS") errors.email = "Ya existe un usuario con ese correo electronico."; else errors.email = "No fue posible validar el correo. Intenta nuevamente."; return false; } finally { validatingEmail.value = false; } }
function validate() { resetErrors(); if (!form.name) errors.name = "El nombre es obligatorio."; if (!form.userName) errors.userName = "El nombre de usuario es obligatorio."; validateEmailFormat(); if (form.password && form.password.length < 6) errors.password = "Debe tener al menos 6 caracteres."; if (isCreateMode.value && !form.password) errors.password = "La contrasena inicial es obligatoria."; if (form.password !== form.passwordConfirm) errors.passwordConfirm = "Las contrasenas no coinciden."; if (form.signaturePassword && form.signaturePassword.length < 6) errors.signaturePassword = "Debe tener al menos 6 caracteres."; if (form.signaturePassword !== form.signaturePasswordConfirm) errors.signaturePasswordConfirm = "Las claves no coinciden."; return Object.keys(errors).length === 0; }
function buildPayload() { const payload = { name: form.name, user_name: form.userName, email: normalizeEmail(form.email), telephone: form.telephone, direction: form.direction || null, position: form.position || null, college_number: form.collegeNumber || null }; if (form.password) payload.password = form.password; if (form.signaturePassword) payload.passwordSignature = form.signaturePassword; if (isCreateMode.value) payload.hide_user = false; return payload; }
async function submitIdentity() { if (!validate() || saving.value) return; if (!(await validateRemoteEmail())) return; saving.value = true; formError.value = ""; try { const payload = buildPayload(); const action = isCreateMode.value ? "created" : "updated"; let saved = action === "created" ? await createUser(payload) : await updateUser(currentUser.value.id, payload); if (photoFile.value) await uploadUserAsset(photoFile.value, "photos", saved.id); if (signatureFile.value) await uploadUserAsset(signatureFile.value, "signatures", saved.id); if (photoFile.value || signatureFile.value) saved = await updateUser(saved.id, {}); close(identityDialog); emit("saved", { user: saved, action }); await nextTick(); toast.success(action === "created" ? "Usuario creado correctamente." : "Usuario actualizado correctamente."); } catch (error) { const code = error?.message || ""; if (code === "USER_EMAIL_INVALID") { errors.email = "Ingresa un correo valido, por ejemplo usuario@dominio.com."; formError.value = ""; } else if (code === "USER_EMAIL_ALREADY_EXISTS" || /correo/i.test(code) && /existe|duplic/i.test(code)) { errors.email = "Ya existe un usuario con ese correo electronico."; formError.value = ""; } else { formError.value = code || "No fue posible guardar el usuario."; toast.error(formError.value); } } finally { saving.value = false; } }
async function submitStateChange(user) { if (user) stateTarget.value = user; if (!stateTarget.value || savingState.value) return; savingState.value = true; try { const saved = await updateUser(stateTarget.value.id, { hide_user: !stateTarget.value.hidden }); const action = saved.hidden ? "deactivated" : "reactivated"; toast.info(action === "deactivated" ? "Usuario desactivado correctamente." : "Usuario reactivado correctamente."); stateDialog.value?.close(); emit("saved", { user: saved, action }); } catch (error) { stateDialog.value?.setError(error?.message || "No fue posible cambiar el estado."); toast.error(error?.message || "No fue posible cambiar el estado."); } finally { savingState.value = false; } }
